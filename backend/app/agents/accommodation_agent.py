...
class AccommodationAgent(BaseAgent):
    ...
    description: ClassVar[str] = (
        "Specializes in finding and ranking accommodations like hotels and guesthouses."
    )
    ...
    def system_prompt(self) -> str:
        return (
            "You are an accommodation specialist. Search for hotels/guesthouses. "
            "Compare by price, rating, location proximity to activities. "
            "NEVER invent prices. Return ranked options.\n\n"
    ...
    def _build_user_prompt(self, context: AgentContext) -> str:
        ...
        prior_results_str = json.dumps(
            context.prior_results.get("places", {}), indent=2, default=str
        )
        if len(prior_results_str) > 2000:
            prior_results_str = prior_results_str[:2000] + "... (truncated)"

        return (
            "Find optimal accommodation options based on the following details:\n\n"
            f"Trip Data:\n{trip_data_str}\n\n"
            f"Preferences:\n{prefs_str}\n\n"
            f"Prior Places Data:\n{prior_results_str}\n\n"
            "Extract destination, dates, num_travelers, and budget allocation for "
            "accommodation to execute your search. Use tools to find real options, "
            "considering proximity to activities if places data is available."
        )
